...
class Sales extends CI_Controller {
    ...
    function index($cDesde='null', $cHasta='null', $cStore_id='null') {
        ...
        $this->template->load('view_layout_modern', 'sales/index', $this->data);
    }
    ...
    function get_sales($cDesde,$cHasta,$cStore){
        ...
        round(total,2) total, round(grand_total,2) grand_total,  tec_users.username created_by, tec_sales.anulado,
            ...
            " group by tec_sales.id, tec_stores.name, tec_sales.date, customer_name, round(total,2), round(grand_total,2),  tec_users.username, tec_sales.anulado, 
            concat(tec_sales.serie,'-',tec_sales.correlativo), 
    ...
    function buscar(){ // LO USA LA BUSQUEDA INCREMENTAL
        ...
        */
        ...
        if($i==0){
            ...
            $cSql = "select a.id, CONCAT(a.name,' ',a.marca,' ',a.modelo) as nombres, a.color, if(b.stock is null,0,b.stock) stock, c.name categoria, a.impuesto, a.prod_serv from tec_products a 
            left join tec_prod_store b on a.id=b.product_id and b.store_id = {$store_id}
            left join tec_categories c on a.category_id=c.id 
            where a.activo='1' and CONCAT(a.name,' ',a.marca,' ',a.modelo) like '%{$code1}%'
            and a.category_id != 9000
            order by CONCAT(a.name,' ',a.marca,' ',a.modelo)";
        }
        if($i>=1){
            $cSql = "select a.id, a.name, a.marca, a.modelo, a.color, if(b.stock is null,0,b.stock) stock, c.name categoria, a.impuesto, a.prod_serv from tec_products a 
            left join tec_prod_store b on a.id=b.product_id and b.store_id = {$store_id}
            left join tec_categories c on a.category_id=c.id 
            where a.activo='1' and ((a.name like '%{$code1}%' and a.marca like '%{$code2}%') or (a.name like '%{$code2}%' and a.marca like '%{$code1}%'))
            and a.category_id != 9000
            order by a.name, a.marca, a.modelo";
            ...
            left join tec_prod_store b on a.id=b.product_id and b.store_id = {$store_id}
            left join tec_categories c on a.category_id=c.id 
            where a.activo='1' and (CONCAT(a.name,' ',a.marca,' ',a.modelo) like '%{$code1}%' and CONCAT(a.name,' ',a.marca,' ',a.modelo) like '%{$code2}%')
            and a.category_id != 9000
            order by CONCAT(a.name,' ',a.marca,' ',a.modelo)";
        }
